...
Follow the prerequisites step first. Then you can jump to either joining mainnet or creating a private chain.
...
minikube
helm
A ZeroTier network with api access token
Installing prerequisites
...
Create a new network by clicking on the "Create a Network" button. Save the 16 character generated network id, e.g. 1c33c1ced02a5eee on your computer.
...
./${CHAIN_NAME}_invite_values.yaml
...
Or if you previously spun up the chain using mkchain, you may adjust your setup to an arbitrary number of nodes by updating the "nodes" section in the values yaml file.
...
config: The config property should mimic the structure of a node's config.json. Run tezos-node config --help for more info.
defaults are filled in for most values.

Each statefulset can run either Nomadic Lab's tezos-node or TezEdge's tezedge node. Either can support all of the other containers. If you specify tezedge as one of the containers to run, then it will be run in preference to tezos-node.
...
tezedge-full-node-1
tezedge-full-node-2
baking-node-0 will run baker, endorser, and logger containers and will be the only bootstrap node. full-node-* are just nodes with no extras. tezedge-full-node-* will be tezedge nodes running baker, endorser, and logger containers.
...
Adding external nodes to the cluster
External nodes to your local cluster can be added to your network by sharing a yaml file generated by the mkchain command.
...
Follow the prerequisite installation instructions
Start minikube
Then run:
...
TzKT
Look here in the Tezos Helm chart's values.yaml indexer section for how to deploy an indexer.
