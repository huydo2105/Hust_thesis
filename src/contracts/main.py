    ...
    class ShardingBlockchain(sp.Contract):
        ...
        def select_leaders(self):
            ...
            for shard in self.data.proposed_leaders.items():
                proposed_leaders = shard.value.elements()
                for leader in proposed_leaders:
                    if x == 0:
                        selected_leader = leader
                        self.data.leaders = sp.update_map(shard.key, sp.Some(selected_leader), self.data.leaders)
                        x += 1
                    else:
                        sp.send(leader, sp.tez(6000))

        @sp.entry_point
        def propose_leader_change(self, params):
            assert sp.amount == sp.tez(6000), "InsufficientStake"
            proposal = sp.record(
                proposer= sp.sender,
        ...
        @sp.entry_point
        def vote_leader_change(self, params):
...
def test():
    ...
    c1.select_leaders().run(valid=True, sender=admin)
    ...
    c1.propose_leader_change(shard="Shard-2", new_leader = nodeF_shard2.address).run(valid=True, sender=nodeF_shard2, amount=sp.tez(6000))

    scenario.h3("Other nodes agree the proposal")
    c1.vote_leader_change(shard="Shard-2").run(valid=True, sender=nodeG_shard2)
    c1.vote_leader_change(shard="Shard-2").run(valid=True, sender=nodeH_shard2)
