...
def run_algo():
    ...
    # Define the command
    node_command = ['./script/generate_values.sh', '-b', '1', '-n', str(state['num_nodes'])]
    protocol_command = ['.script/protocol.sh', '-b', str(state['block_size']), '-p', str(state['punishment']), 'e', str(state['reward_block'])]
    # update_command = 
    # Run the command
