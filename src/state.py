...
def process_state(chain_name):
    with open(f'./{chain_name}_values.txt', 'r') as f:
        data = f.read()
    ...
    num_nodes = np.array([total_instances])
    ...
    # Scale features
    num_nodes_scaled = (2 * (num_nodes - MIN_NUM_NODES) / (MAX_NUM_NODES - MIN_NUM_NODES)) - 1
    ...
    state = np.concatenate([block_size_scaled, finance, memory_capacity_scaled, cpu_capacity_scaled, requirement_feature, num_nodes_scaled])
    return state
